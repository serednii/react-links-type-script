export const DATA_MENU = [
  { menu: 'Java', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти'] },
  { menu: 'Python', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти', 'Масиви', 'Функції'] },
  { menu: 'C++', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти', 'Масиви', 'Функції'] },
  { menu: 'Go', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти', 'Масиви', 'Функції'] },
  { menu: 'SQL', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти', 'Масиви', 'Функції'] },
  { menu: 'MongoDB', arraySubmenu: ['Varssssss', 'цикли', 'Обєкти', 'Масиви', 'Функції'] }
]

export const DATA_LINKS = {
  'Java1': null
  ,
  'Varssssss': [
    {
      link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
      name: 'Переменные в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/peremennie-v-java',
      name: 'Переменные в Java и константы'
    },
    {
      link: 'https://metanit.com/java/tutorial/2.1.php',
      name: 'Переменные и константы'
    },
    {
      idArticle: 1,
      name: 'Переменные и константы',
    },
    {
      idArticle: 1,
      name: 'Переменные и константы',
    }]
  ,
  'Java': {
    'Java1': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'Varssssss1': null
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Java2': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Java3': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Varssssss': [
      {
        link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
        name: 'Переменные в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/peremennie-v-java',
        name: 'Переменные в Java и константы'
      },
      {
        link: 'https://metanit.com/java/tutorial/2.1.php',
        name: 'Переменные и константы'
      }]
    ,
    'цикли': [
      {
        link: 'https://metanit.com/java/tutorial/2.6.php',
        name: 'Циклы'
      },
      {
        link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
        name: 'nКак использовать циклы в языке Java. Полное руководствоame'
      },
      {
        link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
        name: 'Циклы в Java: основы работы, как использовать и примеры'
      }]
    ,

    'Обєкти': [{
      link: 'https://metanit.com/java/tutorial/3.1.php',
      name: 'Классы и объекты'
    },
    {
      link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
      name: 'Объекты в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
      name: 'Знакомство с классами: написание собственных классов, конструкторы'
    },
    {
      link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
      name: 'name'
    }]

  }
  ,
  'Python': {
    'Varssssss': [
      {
        link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
        name: 'Переменные в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/peremennie-v-java',
        name: 'Переменные в Java и константы'
      },
      {
        link: 'https://metanit.com/java/tutorial/2.1.php',
        name: 'Переменные и константы'
      }]
    ,
    'цикли': [
      {
        link: 'https://metanit.com/java/tutorial/2.6.php',
        name: 'Циклы'
      },
      {
        link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
        name: 'nКак использовать циклы в языке Java. Полное руководствоame'
      },
      {
        link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
        name: 'Циклы в Java: основы работы, как использовать и примеры'
      }]
    ,
    'Обєкти': [{
      link: 'https://metanit.com/java/tutorial/3.1.php',
      name: 'Классы и объекты'
    },
    {
      link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
      name: 'Объекты в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
      name: 'Знакомство с классами: написание собственных классов, конструкторы'
    },
    {
      link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
      name: 'name'
    }]

  }
  ,

  'C++': {
    'Varssssss': [
      {
        link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
        name: 'Переменные в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/peremennie-v-java',
        name: 'Переменные в Java и константы'
      },
      {
        link: 'https://metanit.com/java/tutorial/2.1.php',
        name: 'Переменные и константы'
      }]
    ,
    'цикли': [
      {
        link: 'https://metanit.com/java/tutorial/2.6.php',
        name: 'Циклы'
      },
      {
        link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
        name: 'nКак использовать циклы в языке Java. Полное руководствоame'
      },
      {
        link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
        name: 'Циклы в Java: основы работы, как использовать и примеры'
      }]
    ,
    'Обєкти': [{
      link: 'https://metanit.com/java/tutorial/3.1.php',
      name: 'Классы и объекты'
    },
    {
      link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
      name: 'Объекты в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
      name: 'Знакомство с классами: написание собственных классов, конструкторы'
    },
    {
      link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
      name: 'name'
    }]
  }
}

export const DATA_LINKS1 = {
  'Java1': null
  ,
  'Varssssss': [
    {
      link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
      name: 'Переменные в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/peremennie-v-java',
      name: 'Переменные в Java и константы'
    },
    {
      link: 'https://metanit.com/java/tutorial/2.1.php',
      name: 'Переменные и константы'
    },
    {
      idArticle: 1,
      name: 'Переменные и константы',
    },
    {
      idArticle: 1,
      name: 'Переменные и константы',
    }]
  ,
  'Java': {
    'Java1': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'Varssssss1': null
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Java2': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Java3': {
      'Varssssss': [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        }]
      ,
      'цикли': [
        {
          link: 'https://metanit.com/java/tutorial/2.6.php',
          name: 'Циклы'
        },
        {
          link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
          name: 'nКак использовать циклы в языке Java. Полное руководствоame'
        },
        {
          link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
          name: 'Циклы в Java: основы работы, как использовать и примеры'
        }]
      ,

      'Обєкти': [{
        link: 'https://metanit.com/java/tutorial/3.1.php',
        name: 'Классы и объекты'
      },
      {
        link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
        name: 'Объекты в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
        name: 'Знакомство с классами: написание собственных классов, конструкторы'
      },
      {
        link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
        name: 'name'
      }]

    },
    'Varssssss': [
      {
        link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
        name: 'Переменные в Java'
      },
      {
        link: 'https://javarush.com/groups/posts/peremennie-v-java',
        name: 'Переменные в Java и константы'
      },
      {
        link: 'https://metanit.com/java/tutorial/2.1.php',
        name: 'Переменные и константы'
      }]
    ,
    'цикли': [
      {
        link: 'https://metanit.com/java/tutorial/2.6.php',
        name: 'Циклы'
      },
      {
        link: 'https://skillbox.ru/media/base/kak-ispolzovat-tsikly-v-yazyke-java-polnoe-rukovodstvo/',
        name: 'nКак использовать циклы в языке Java. Полное руководствоame'
      },
      {
        link: 'https://timeweb.cloud/tutorials/java/cikly-v-java-osnovy-raboty',
        name: 'Циклы в Java: основы работы, как использовать и примеры'
      }]
    ,

    'Обєкти': [{
      link: 'https://metanit.com/java/tutorial/3.1.php',
      name: 'Классы и объекты'
    },
    {
      link: 'https://javarush.com/quests/lectures/questsyntaxpro.level07.lecture03',
      name: 'Объекты в Java'
    },
    {
      link: 'https://javarush.com/groups/posts/1949-znakomstvo-s-klassami-napisanie-sobstvennihkh-klassov-konstruktorih',
      name: 'Знакомство с классами: написание собственных классов, конструкторы'
    },
    {
      link: 'https://skillbox.ru/media/base/klassy-i-obekty-v-java/',
      name: 'name'
    }]

  }
  ,

}


const newStrucrure = {
  level: 0,
  type: null,

}

const newStrucrure_1 = {
  level: 0,
  type: "menu",
  menu: [{
    level: 1,
    nameMenu: "java",
    type: null
  },
  {
    level: 1,
    nameMenu: "js",
    type: 'menu',
    menu: [{
      level: 2,
      nameMenu: "java",
      type: null
    },
    {
      level: 2,
      nameMenu: "js",
      type: 'menu'

    },]
  },
  {
    level: 1,
    nameMenu: "java",
    type: "idLinks",
    idLinks:



      [
        {
          link: 'https://vertex-academy.com/tutorials/ru/sozdanie-peremennyx-i-tipy-peremenny/',
          name: 'Переменные в Java'
        },
        {
          link: 'https://javarush.com/groups/posts/peremennie-v-java',
          name: 'Переменные в Java и константы'
        },
        {
          link: 'https://metanit.com/java/tutorial/2.1.php',
          name: 'Переменные и константы'
        },
        {
          idArticle: 1,
          name: 'Переменные и константы',
        },
        {
          idArticle: 1,
          name: 'Переменные и константы',
        }

      ]


  },
  ]

}

//1: Створити базу даних в graphQL
//2: Маємо обєкт поля якого є пункти меню
//3: Кожен пункт меню вміщує null, {}, []
//4: null то пусте меню
//5: {} обєкт в собі має дальші підменю
//6: [] масив має обєкти з полями link, idArticle, name
